    ...
    def toggle(self, window):
        ...
        for lign in self.structure:
            ...
            for sprite in lign:
                ...
                pixel_x = num_case * size_sprite
                pixel_y = num_case * size_sprite
                if sprite == 'w':	# w = wall
                    window.blit(wall, (pixel_x, pixel_y))
                if sprite == 'e':
                    window.blit(guard, (pixel_x, pixel_y))
                num_case += 1
            num_lign += 1
...
class MacGyver:
        ...
        if direction == 'right':
            ...
            if self.case_x < (number_sprite_width - 1):
                ...
                if self.level.structure[self.case_y][self.case_x+1] != 'w':
                    ...
                    self.pixel_x = self.case_x * size_sprite
        ...
        if direction == 'top':
            if self.case_y > 0:
                if self.level.structure[self.case_y-1][self.case_x] != 'w':
                    self.case_y -= 1
                    self.pixel_y = self.case_y * size_sprite
    ...
    def inventory(self):
        """Add a number of items in inventory"""
...
class Item:
    ...
    def randpos(self):
        ...
        while True:
            ...
            if self.level.structure[self.case_y][self.case_x] == '0':
                self.level.structure[self.case_y][self.case_x] = self.identification
                break
        return self.case_x, self.case_y

    def damage(self):
        """Methode damage item on structure"""
        self.health -= 1
    ...
    def display(self, window):
        ...
        if self.health > 0:
            window.blit(self.sprite, (self.pixel_x, self.pixel_y))
